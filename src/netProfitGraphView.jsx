...
function ViewNetProfitGraph({ projectId, graphType }) {
  ...
  const transformGraphData = (graphData) => {
    ...
    const categories = graphData.years[0].months.map(
    ...
    const options = {
      ...
      xaxis: {
        categories: categories.map((month) => [month]), // Ensure each month is wrapped in an array
        labels: {
          style: {
            fontSize: "12px"
          }
    ...
    try {
      ...
      if (element) {
        ...
        canvas.toBlob(async (blob) => {
          ...
          formData.append("type", "Net Profit");
...
export default ViewNetProfitGraph;
