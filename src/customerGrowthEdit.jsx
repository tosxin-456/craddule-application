...
function OperatingIncomeEdit() {
  ...
  const addYear = () => {
    ...
    setMonthInputs((prev) => ({
      ...prev,
      [`year${newYear.year}`]: newYear.months.reduce((acc, m) => {
        acc[m.month] = m.value;
        return acc;
      }, {})
    }));
  ...
  const removeYear = () => {
    if (yearsData.length > 1) {
      ...
      const updated = { ...monthInputs };
      delete updated[`year${yearsData.length}`];
      setMonthInputs(updated);
    }
  };
        ...
        if (response.ok) {
          ...
          const formattedYears = data.years.map((year) => ({
            ...
            months: [
              ...
              const match = year.months.find((m) => m.month === month);
          ...
          formattedYears.forEach((year) => {
            inputMap[`year${year.year}`] = year.months.reduce((acc, m) => {
              acc[m.month] = m.value;
              return acc;
            }, {});
          });
      ...
      <div className="px-10 py-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">Edit your graph data below</p>
          </div>
...
export default OperatingIncomeEdit;
